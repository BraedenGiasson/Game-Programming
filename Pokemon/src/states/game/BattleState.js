...
export default class BattleState extends State {
	...
	constructor(player, opponent) {
		...
		this.initialPlayerHealth = this.playerPokemon.health;
		this.initialOpponentHealth = this.opponentPokemon.health;
		...
		this.playerExperienceBar = new ProgressBar(
			Panel.BATTLE_PLAYER.x + 1, 
			Panel.BATTLE_PLAYER.y + 2.56,
			ProgressBar.WIDTH, 
			ProgressBar.HEIGHT,
			false,
			Colour.Blue,
			Panel.BATTLE_PLAYER.x - 2.5,
			false,
			true
		);
	}
	...
	renderForeground() {
		...
		this.playerExperienceBar.render();
	}
	...
	triggerBattleStart() {
		...
		// timer.tween(this.opponentPokemon.position, ['x'], [Pokemon.FRONT_POSITION.end.x], 0.75, () => this.sendOutPlayerPokemon());
