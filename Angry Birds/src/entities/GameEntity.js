...
export default class GameEntity {
	...
	static EXPLOSION_SPRITE_MEASUREMENTS = [
		{ x: 169, y: 513, width: 115, height: 111 },
		{ x: 169, y: 400, width: 113, height: 110 }, //grey
		{ x: 169, y: 277, width: 126, height: 122 }, // white
		{ x: 169, y: 155, width: 126, height: 122 }, // disappear white
	];
	...
	render(renderDebug, scaleAmount) {
		context.save();
		...
		if (this.animation !== null){
			// Render sprite normally
			if (scaleAmount === null){
				this.sprites[this.animation.getCurrentFrame()].render(this.renderOffset.x, this.renderOffset.y);
			}
			// If the scale amount is specified, render the sprite by the scale
			else{
				this.sprites[this.animation.getCurrentFrame()].render(this.renderOffset.x, this.renderOffset.y, scaleAmount);
			}
		}
		// Render just the current frame of the sprite
		else{
