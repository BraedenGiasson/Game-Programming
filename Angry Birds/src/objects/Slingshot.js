...
export default class Slingshot {
	...
	update(dt) {
		...
		// Make sure the space bar was only pressed once
		if (!this.didPressSpacebar && this.bird.isYellowBird()){
			// True to accelerate yellow bird
			if (this.bird.checkToAccelerate(this.wasLaunched)){
				this.bird.accelerate();
				this.didPressSpacebar = true;
			}
		}

		if (this.shouldUnload()) {
			// Checks if the bird is the black bird and if it's in its base state
			if (this.bird.isBlackBird()
				&& this.bird.stateMachine.currentState.name === BirdStateName.Base) 
			{
				this.bird.stateMachine.change(BirdStateName.ChargeUp);
			}
			// If the bird is ready to unload, unload the bird
			else if (this.bird.shouldCleanUp) {
				this.unload();
			}
		}
