...
export default class Fortress {
	...
	update(dt) {
		...
		this.explosionAnimations.forEach((explosion) => explosion.update(dt));

		// Filter out already finished explosions
		this.explosionAnimations = this.explosionAnimations.filter((explosion) => !explosion.shouldCleanUp);
	}

	render() {
		[...this.blocks, ...this.pigs].forEach((entity) => entity.render());
		this.explosionAnimations.forEach((explosion) => explosion.render());
	}
	...
	removeBodies() {
		this.bodiesToRemove.forEach((body) => {
			...
			// Add a new explosion for the broken block 
			if (body.entity instanceof Block) {
				this.explosionAnimations.push(new Explosion(
					body.position.x,
					body.position.y,
					0.08
				))
